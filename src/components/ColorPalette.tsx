...
const ColorPalette = memo(({ colors, onCopyColor }: ColorPaletteProps) => {
  ...
  const handleCopyColor = useCallback(
    (color: string, format: string = "hex") => {
      onCopyColor(color);
      setCopiedColor(color);
      trackCodeCopy(format);
  ...
  const rgbToLab = useCallback((r: number, g: number, b: number) => {
    const x = r * 0.412453 + g * 0.35758 + b * 0.180423;
    const y = r * 0.212671 + g * 0.71516 + b * 0.072169;
    const z = r * 0.019334 + g * 0.119193 + b * 0.950227;

    const l =
      y > 0.008856 ? 116 * Math.pow(y / 100, 1 / 3) - 16 : (903.3 * y) / 100;
    const a = 500 * (Math.pow(x / 95.047, 1 / 3) - Math.pow(y / 100, 1 / 3));
    const bLab =
      200 * (Math.pow(y / 100, 1 / 3) - Math.pow(z / 108.883, 1 / 3));
            ...
            <div
              ...
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-300 rounded-lg flex items-center justify-center">
                {copiedColor === color.hex ? (
                  <Check
                    className={cn(
                      "w-4 h-4 sm:w-5 sm:h-5 opacity-100",
                      color.contrast === "dark" ? "text-white" : "text-gray-900"
                    )}
                  />
                ) : (
                  <Copy
                    className={cn(
                      "w-4 h-4 sm:w-5 sm:h-5 opacity-0 group-hover:opacity-100 transition-opacity duration-300",
                      color.contrast === "dark" ? "text-white" : "text-gray-900"
                    )}
                  />
                )}
      ...
      <div className="mt-6">
        <Tabs defaultValue="hex" className="w-full">
          <TabsList className="grid w-full grid-cols-5">
            ...
            <TabsTrigger value="cmyk" className="text-xs sm:text-sm">
              CMYK
            </TabsTrigger>
            ...
            {colors.map((color, index) => {
              ...
              return (
                <div
                  ...
                  onClick={() =>
                    handleCopyColor(formats?.hex || color.hex, "hex")
                  }>
          ...
          <TabsContent value="rgb" className="space-y-2 mt-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <p className="text-xs text-gray-600">
                Canvas, CSS filters, digital design
              </p>
            </div>
            {colors.map((color, index) => {
              const formats = colorFormats[index];
              return (
                <div
                  ...
                  className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer"
                  onClick={() =>
                    handleCopyColor(formats?.rgb || color.rgb, "rgb")
                  }>
                  <div className="flex items-center space-x-3">
                    <div
                      className="w-6 h-6 rounded-full border-2 border-white shadow-sm"
            ...
            {colors.map((color, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer"
                  onClick={() => handleCopyColor(formats?.hsl || "", "hsl")}>
                  <div className="flex items-center space-x-3">
                    <div
                      className="w-6 h-6 rounded-full border-2 border-white shadow-sm"
                      style={{ backgroundColor: color.hex }}
            ...
            {colors.map((color, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer"
                  onClick={() => handleCopyColor(formats?.cmyk || "", "cmyk")}>
                  <div className="flex items-center space-x-3">
                    <div
                      className="w-6 h-6 rounded-full border-2 border-white shadow-sm"
                      style={{ backgroundColor: color.hex }}
            ...
            {colors.map((color, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer"
                  onClick={() => handleCopyColor(formats?.lab || "", "lab")}>
                  <div className="flex items-center space-x-3">
                    <div
                      className="w-6 h-6 rounded-full border-2 border-white shadow-sm"
                      style={{ backgroundColor: color.hex }}
